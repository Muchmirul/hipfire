#!/usr/bin/env bash
# tools/autokernel-rdna/kernel_lab/harnesses/gemv_hfq6g256_harness.sh
#
# Fixed standalone harness for gemv_hfq6g256 kernel candidates.
#
# NOTE: HFQ6 group layout is different from HFQ4:
#   - 6 bits per weight, 256 weights/group
#   - 192 bytes payload (6*256/8) + 8 bytes header = 200 bytes/group
#   - Packed storage: 3 nibbles per byte-pair (or 4 weights per 3 bytes)
#   - Actual bit-packing scheme is defined in kernels/src/gemv_hfq6g256.hip
#
# Usage:
#   CANDIDATE=tools/autokernel-rdna/kernel_lab/generated/gemv_hfq6g256/candidate_1.hip \
#     bash tools/autokernel-rdna/kernel_lab/harnesses/gemv_hfq6g256_harness.sh
#
# Output:
...
BUILD_DIR="$LAB_DIR/generated/gemv_hfq6g256"
REF_KERNEL="$REPO_ROOT/kernels/src/gemv_hfq6g256.hip"
...
if [ ! -f "$REF_KERNEL" ]; then
    echo "CORRECTNESS: FAIL"
    echo "ERROR: Reference kernel not found: $REF_KERNEL (HFQ6 kernels may not exist yet)"
    exit 2
fi
...
# NOTE: This driver is a stub. The HFQ6 bit-packing differs from HFQ4.
# The actual dequant reference must match kernels/src/gemv_hfq6g256.hip exactly.
# Read that source to verify the 6-bit packing scheme before writing the CPU reference.
...
if [ ! -f "$REF_BIN" ] || [ "$REF_KERNEL" -nt "$REF_BIN" ]; then
    ...
    if [ $? -ne 0 ]; then
        echo "CORRECTNESS: FAIL"
        echo "ERROR: Reference kernel failed to compile — HFQ6 packing mismatch in CPU reference?"
        exit 2
    fi
fi
